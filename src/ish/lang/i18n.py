...
class I18N:
	...
	def _get_lang_file(self, lang: Optional[str]) -> Path:
		...
		try:
			...
			logging.getLogger('global').warning('Cannot create language file %s: %s', default_file, exc)
	...
	def load_messages(self, lang: Optional[str] = None):
		...
		paths = dict.fromkeys([self.base_path / f'{self.default_lang}.json', selected_path])
		for path in paths:
			if not path.exists():
				continue
			try:
				...
				logging.getLogger('global').warning('Cannot load language file %s: %s', path, exc)
		with self._lock:
