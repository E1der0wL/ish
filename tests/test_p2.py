"""Regressions for parsing, memory, hooks, FD ownership, dependencies, and translations."""
...
from ish.shell.integration import AFTER_PROMPT, BEFORE_PROMPT
...
class ParsingTests(unittest.TestCase):
    """Verify argument boundaries in aliases, internal argv, and completion contexts."""

    def test_alias_names_quotes_newlines_and_equals(self):
        """Preserve alias names and spaces, newlines, and equals signs in quoted alias
        bodies.
        """
        aliases = {
            "ll": "ls -l",
        ...
        data = "\n".join(
            "alias " + k + "=" + shlex.quote(v) for k, v in aliases.items()
        )
        self.assertEqual(alias_parser(data.encode(), "utf-8"), aliases)
        self.assertEqual(
            alias_parser(b"ll='ls -l'\nx='echo hi'", "utf-8", "zsh"),
            {"ll": "ls -l", "x": "echo hi"},
        )
        self.assertEqual(
            alias_parser(b"ll\tls -l\nx\techo hi\n", "utf-8", "csh"),
    ...
    def test_literal_argv_and_shell_syntax(self):
        """Split literal internal-tool arguments while leaving compound shell syntax to the
        shell.
        """
        self.assertEqual(
            simple_command("tool \"two words\" '' a\\ b 'x;y'"),
            ["tool", "two words", "", "a b", "x;y"],
        )
        for text in [
            "tool x | cat",
            "tool && true",
    ...
    def test_completion_preserves_one_argument(self):
        """Complete paths containing special characters as single shell arguments."""
        with tempfile.TemporaryDirectory() as directory:
            ...
            for prefix in ["cat hel", 'cat "hel', "cat 'hel", "cat hello\\ "]:
                ...
                for completion in results:
                    completed = (
                        prefix[: len(prefix) + completion.start_position]
                        + completion.text
                    )
    ...
    def test_completion_operators_case_and_substitution(self):
        """Offer candidates matching operator and substitution context and case policy."""
        ...
        for prefix in [
            ...
            self.assertEqual(
                list(completer.get_completions(Document(prefix), CompleteEvent())),
                [],
                prefix,
            )
...
class MemoryTests(unittest.TestCase):
    """Verify memory bounds and resumption for output queues, history, and partial
    controls.
    """

    def test_pending_write_queue_is_bounded_and_signals_backpressure(self):
        """Honor output limits and signal backpressure as queues grow and drain."""
        loop, flow = Mock(), Mock()
    ...
    def test_history_counts_and_total_budget(self):
        """Keep complete lines, partial lines, and last output within the combined byte
        budget.
        """
    ...
    def test_unterminated_prompt_and_controls_are_bounded_and_recover(self):
        """Bound unfinished prompt and control-sequence memory and recognize the next
        boundary.
        """
    ...
    def test_unicode_fragmentation_and_between_passthrough(self):
        """Preserve original bytes across Unicode splits and non-removing prompt captures."""
        data = "가나다🙂".encode()
        sequencer = Sequencer()
        self.assertEqual(b"".join(sequencer.interpret(bytes([v])) for v in data), data)
        seen = []
        sequencer.between_sequence(
            BEFORE_PROMPT, AFTER_PROMPT, seen.append, remove_seq=False
        )
...
class PromptTests(unittest.IsolatedAsyncioTestCase):
    """Verify contracts between the editor, workers, hooks, state FIFO, and caller-owned
    FDs.
    """

    async def test_worker_slow_output_keeps_tail_with_bounded_queue(self):
        """Drain final worker output with a small queue and a slow consumer."""
        ...
        def writer(*args, **kwargs):
            """Use a writer with small limits to simulate a slow output consumer."""
        ...
        try:
            with patch("ish.app.pytool.FDWriter", side_effect=writer):
                task = asyncio.create_task(
                    ProcessHandler(stdin=input_r, stdout=output_w).run(
                        print, "x" * 200000 + "END"
                    )
                )
    ...
    async def test_hooks_pass_callable_and_fallback_requires_error(self):
        """Pass actual callables to hooks and invoke fallback only on failure status."""
        prompt = SimpleNamespace(
            ...
            logger=Mock(),
            interactive_shell=SimpleNamespace(last_command="false"),
            context=SimpleNamespace(exitcode="1"),
        )
        await Prompt.pre_exec(prompt)
        await Prompt.post_exec(prompt)
        await Prompt.fallback(prompt)
        self.assertEqual(
            [c.args for c in prompt.process_handler.run.await_args_list],
            [(print,), (len,), (repr,)],
        )
    ...
    async def test_internal_tool_with_quoted_arguments(self):
        """Dispatch internal commands with quoted arguments to tools instead of the shell."""
        prompt = SimpleNamespace(
            prompt_async=AsyncMock(return_value="tool \"two words\" '' a\\ b"),
            exit_command="ish_exit",
            internal_commands={},
            internal_tools={"tool": print},
            process_handler=SimpleNamespace(run=AsyncMock()),
        )
        self.assertEqual(await Prompt.get(prompt), "")
        prompt.process_handler.run.assert_awaited_once_with(
            print, "two words", "", "a b"
        )
    ...
    async def test_custom_fds_reach_both_engines_and_remain_owned_by_caller(self):
        """Pass supplied FDs to the shell and worker without closing them when UI streams
        detach.
        """
        ...
        try:
            with tempfile.TemporaryFile(mode="w+") as out:
                with patch("ish.ui.prompt.get_builtins", return_value=[]):
                    prompt = Prompt(
                        shell="bash", input_fd=slave, output_fd=out.fileno()
                    )
    ...
    async def test_context_wait_handles_prompt_arriving_before_fifo(self):
        """Wait for the matching state frame even when the prompt arrives first."""
...
class DependencyTests(unittest.TestCase):
    """Verify version constraints, distribution and import names, and pip argument vectors."""

    def test_pep440_ranges_and_invalid_specs(self):
        """Parse compound PEP 440 constraints and reject invalid version constraints."""
    ...
    def test_install_uses_argv_without_shell(self):
        """Pass each installation request as one pip argument without shell interpretation."""
        manager = PluginManager()
        manager.python_exe = "/tmp/python with spaces"
        with patch(
            "ish.plugin.manager.subprocess.run",
    ...
    def test_distribution_version_uses_distribution_name(self):
        """Look up installed versions by distribution name when the import name differs."""
        manager = PluginManager()
        with (
            patch("ish.plugin.manager.importlib.util.find_spec", return_value=object()),
            patch(
                "ish.plugin.manager.importlib.metadata.version", return_value="1.5"
            ) as version,
        ):
...
class LanguageTests(unittest.TestCase):
    """Verify fallback behavior for missing translations, language changes, and malformed
    JSON.
    """

    def test_partial_translation_fallback_and_switch(self):
        """Fill missing translations with defaults and discard stale values when changing
        languages.
        """
        with tempfile.TemporaryDirectory() as directory:
            lang = I18N()
    ...
    def test_invalid_language_logs_and_keeps_defaults(self):
        """Log a malformed translation file and retain default messages."""
        with tempfile.TemporaryDirectory() as directory:
            lang = I18N()
            lang.base_path = Path(directory)
