...
from ish.shell.protocol import FrameDecoder, SOH, RS, US, EOT, VERSION
from ish.ui.prompt import Prompt
from p1_workers import terminal_echo, large_output, wait_forever, exit_seven


def frame(category, data, versioned=True):
    if versioned:
        return SOH + VERSION + RS + category.encode() + US + base64.b64encode(data) + EOT
...
class WriterTests(unittest.IsolatedAsyncioTestCase):
    async def test_partial_eintr_and_eagain(self):
        ...
        with patch('ish.fdio.os.write', side_effect=[2, InterruptedError(), 1, BlockingIOError()]) as write:
            writer.write(b'abcdef')
            self.assertEqual([bytes(c.args[1]) for c in write.call_args_list], [b'abcdef', b'cdef', b'cdef', b'def'])
    ...
    async def test_bad_fd_does_not_retry(self):
        ...
        with patch('ish.fdio.os.write', side_effect=OSError(errno.EBADF, 'bad fd')) as write:
...
class ProtocolTests(unittest.TestCase):
    def test_coalesced_legacy_frames(self):
        ...
        self.assertEqual(decoder.feed(frame('exitcode', b'1', False) + frame('exitcode', b'2', False)), [('exitcode', b'1'), ('exitcode', b'2')])
    ...
    def test_bash_emits_unambiguous_environment(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            subprocess.run(['bash', '--noprofile', '--norc', '-c',
                            ...
                            'test', str(script), str(output)],
                           env={**os.environ, 'P1_VALUE': value}, check=True, timeout=3)
            records = dict(FrameDecoder().feed(output.read_bytes()))
            self.assertEqual(records['exitcode'], b'7')
            self.assertEqual(dict_parser(records['environ'], 'utf-8', '=', '\0')['P1_VALUE'], value)
...
class InitializationTests(unittest.IsolatedAsyncioTestCase):
    async def test_init_yields_and_waits_for_prompt(self):
        ...
        ticks = []
        shell.loop.call_later(.005, ticks.append, True)
        shell.loop.call_later(.02, shell._init_event.set)
        writer = SimpleNamespace(write=Mock(), drain=AsyncMock())
        with patch.object(InteractiveShell, '_send', new=AsyncMock()), patch.object(InteractiveShell, '_writer', return_value=writer):
    ...
    async def test_init_timeout_and_child_exit(self):
        for exited in (False, True):
            ...
            shell._init_event = asyncio.Event()
            shell._fatal_error = shell.loop.create_future()
            shell.proc = SimpleNamespace(wait=AsyncMock(return_value=3) if exited else AsyncMock(side_effect=asyncio.Event().wait))
            with patch.object(InteractiveShell, '_send', new=AsyncMock()):
                with self.assertRaisesRegex(RuntimeError, 'exited' if exited else 'timed out'):
    ...
    async def test_cleanup_on_spawn_failure_init_failure_and_cancel(self):
        for stage in ('fifo', 'spawn', 'init', 'cancel'):
            with self.subTest(stage=stage), tempfile.TemporaryDirectory() as d:
                ...
                shell = InteractiveShell('bash', stdin=slave, stdout=slave, stderr=slave,
                ...
                try:
                    with patch('ish.shell.base.build_binary', return_value=True), patch('ish.shell.base.install_scripts', return_value=Path(d)), patch.object(InteractiveShell, '_init', side_effect=init):
                        if stage == 'fifo':
                            ...
                            with patch.object(InteractiveShell, '_spawn', new=AsyncMock(side_effect=RuntimeError('injected spawn failure'))):
                            ...
                            with self.assertRaises(asyncio.CancelledError):
                                await task
                    for name in ('master_fd', 'slave_fd', 'dupin_fd', 'shell_pipe', 'tty_pipe'):
...
class AlreadyFixedTests(unittest.IsolatedAsyncioTestCase):
    async def test_tab_starts_async_completion_and_applies_selection(self):
        ...
        buffer.complete_state = CompletionState(Document('l'), [Completion('ls ', start_position=-1)], complete_index=0)
        tab(event)
        self.assertEqual(buffer.text, 'ls ')

    async def test_plugin_parse_and_registration_and_unload(self):
        manager = PluginManager()
        self.assertEqual(manager._parse_requirement('pkg>=1|import_name'), ('pkg', '>=1', 'import_name'))
        with tempfile.TemporaryDirectory() as d:
            for entry in ('__init__.py', 'p1_plugin.py'):
                plugin = Path(d) / 'p1_plugin'
                plugin.mkdir(exist_ok=True)
                source = plugin / entry
                source.write_text('PLUGIN_META = {"version": "1.0"}\ndef tool(): return 42\n')
    ...
    async def test_bad_plugin_does_not_hide_original_exception(self):
        ...
        try:
            with tempfile.TemporaryDirectory() as d:
                ...
                plugin.mkdir()
                (plugin / '__init__.py').write_text('PLUGIN_META = {}\nraise RuntimeError("original failure")\n')
                manager = PluginManager()
                manager.logger = Mock()
                self.assertEqual(manager.load(), [])
                self.assertIn('original failure', manager.logger.error.call_args.args[0])
        finally:
...
class WorkerTests(unittest.IsolatedAsyncioTestCase):
    async def run_tool(self, func, data=b'', cancel=False):
        ...
        task = asyncio.create_task(ProcessHandler(stdin=input_r, stdout=output_w).run(func))
        try:
            if cancel:
    ...
    async def test_start_failure_closes_resources_and_restores_flags(self):
        ...
        try:
            ...
            with patch('multiprocessing.process.BaseProcess.start', side_effect=RuntimeError('spawn failure')):
...
class CForwardTests(unittest.TestCase):
    def test_full_fifo_is_retried_without_losing_input(self):
        with tempfile.TemporaryDirectory() as d:
            source, binary, fifo = [Path(d) / name for name in ('forward.c', 'forward', 'input.fifo')]
            source.write_text(TTY_FORWARD)
            subprocess.run(['gcc', '-Wall', '-Wextra', '-Werror', '-O2', '-o', str(binary), str(source)], check=True, timeout=10)
            os.mkfifo(fifo)
            ...
            process = subprocess.Popen([str(binary), str(fifo)], stdin=slave, stdout=subprocess.DEVNULL)
            try:
                time.sleep(.05)
