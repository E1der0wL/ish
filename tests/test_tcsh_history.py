...
class TcshHistoryTests(unittest.TestCase):
    def exercise(self, ish, commands, files, rc='', fixtures=None):
        ...
        with tempfile.TemporaryDirectory(prefix='ish-history-') as directory:
            ...
            (scratch / '.tcshrc').write_text("set prompt = 'READY> '\nset history = 100\n" + rc)
            ...
            def ready():
                if ish:
                    return prompt_text in terminal.output and b'\x1b[?2004h' in terminal.output
            ...
            try:
                ...
                for command in ['history -c', *commands]:
                    ...
                    if pending_input:
                        self.assertTrue(terminal.until(lambda: b'READ_WAITING\r\n' in terminal.output),
                            bytes(terminal.output))
                        # Longer than the old _recover inactivity threshold.
                        terminal.until(lambda: False, 1.5)
                        self.assertFalse((scratch / 'answer').exists(), bytes(terminal.output))
                        terminal.send(pending_input[0] + '\r')
                    self.assertTrue(terminal.until(ready), (command, bytes(terminal.output)))
                    if ish:
                        self.assertNotIn(b'_ish_restore_editor', terminal.output)
                self.assertTrue(terminal.until(lambda: all((scratch / name).exists() for name in files)),
                    bytes(terminal.output))
    ...
    def test_history_argument_selector_matches_native_tcsh(self):
        commands = ['echo first second', 'echo !!:1 > selected', 'history -h > saved-history']
...
alias precmd 'true'
"""
        commands = ['rm -f hook-status hook-commands; history -c', 'false',
            'echo $status > last-status', '/bin/sh run.sh', '!!',
            'history -h > saved-history']
        files = ['hook-status', 'hook-commands', 'last-status', 'result', 'saved-history']
    ...
    def test_skipped_postcmd_keeps_editor_setting(self):
        commands = ['echo ok', '!!:p', '!987654321', 'echo (',
            'echo $?edit > edit-state', 'history -h > saved-history']
    ...
    def test_user_can_change_editor_setting(self):
        commands = ['unset edit', 'echo $?edit >> edit-state', 'echo ok', '!!:p',
            'echo $?edit >> edit-state', 'set edit', 'echo $?edit >> edit-state',
            'history -h > saved-history']
        files = ['edit-state', 'saved-history']
        native = self.exercise(False, commands, files)
    ...
    def test_replace_precmd_at_runtime(self):
        commands = [("alias precmd \"set prompt='ish > '\"", 'ish >'),
            'false', 'echo $status > last-status', '/bin/sh run.sh', '!!',
            'history -h > saved-history']
    ...
    def test_replace_and_remove_postcmd_at_runtime(self):
        commands = ['rm -f hooks; history -c', "alias postcmd 'echo new:$status >> hooks'",
            'false', 'echo $status > last-status', 'unalias postcmd',
            '/bin/sh run.sh', '!!', 'history -h > saved-history']
    ...
    def test_replace_both_hooks_and_remove_precmd(self):
        commands = [("alias precmd \"set prompt='changed > '\"; alias postcmd 'echo $status >> hooks'", 'changed >'),
            '/bin/sh run.sh', 'unalias precmd', '/bin/sh run.sh',
            'echo $?edit > edit-state', 'history -h > saved-history']
    ...
    def test_source_can_replace_both_hooks(self):
        commands = [('source changed.tcsh', 'sourced >'), 'false',
            'echo $status > last-status', '/bin/sh run.sh', '!!',
            'history -h > saved-history']
        fixtures = {'changed.tcsh': "alias precmd \"set prompt='sourced > '\"\n"
        ...
        self.assertEqual(self.exercise(True, commands, files, fixtures=fixtures), native)
    ...
    def test_existing_periodic_hook_and_nested_postcmd_are_preserved(self):
        ...
        commands = ['rm -f hooks periodic-calls; history -c', 'false',
            'echo $status > last-status',
            ("alias precmd \"set prompt='periodic > '\"", 'periodic >'), 'true',
    ...
    def test_existing_periodic_interval_is_preserved(self):
        ...
        commands = ["printf '' > periodic-calls", 'false',
            ("alias precmd \"set prompt='interval > '\"", 'interval >'), 'true']
    ...
    def test_hook_recovery_does_not_inject_into_dollar_less(self):
        commands = [("alias precmd \"set prompt='read > '\"; alias postcmd 'true'", 'read >'),
            ('source read.tcsh', 'read >', 'typed response'), 'history -h > saved-history']
        fixtures = {'read.tcsh': "echo READ_WAITING\nset read_value = \"$<\"\n"
            "printf '%s\\n' \"$read_value\" > answer\n"}
