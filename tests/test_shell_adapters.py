...
from ish.shell.adapters import (
    ADAPTERS,
    CSH_BEHAVIOR,
    SYNTAXES,
    ShellAdapter,
...
class AdapterTests(unittest.TestCase):
    """Verify contracts for reusing syntax and execution policies or extending them to new
    shells.
    """

    def test_source_quotes_paths_and_passes_positional_arguments(self):
        """Safely quote special characters in shell-specific source paths and positional
        arguments.
        """
        ...
        for name in ("bash", "zsh"):
            with self.subTest(shell=name):
                self.assertEqual(
                    shlex.split(get_adapter(name).source(*values)), ["source", *values]
                )
        self.assertEqual(
            get_adapter("tcsh").source(*values),
            "source /tmp/ish\\ user\\'s/init /tmp/a\\ \\$x\\;\\! /tmp/b\\ pipe\n",
        )

    def test_source_binds_variables_for_shells_without_source_arguments(self):
        """Pass equivalent values through variables when a shell does not support source
        arguments.
        """
        values = ("/tmp/init file", "/tmp/in pipe", "/tmp/out pipe")
        self.assertEqual(
            get_adapter("sh").source(*values),
            "_ish_pipe='/tmp/in pipe'; _tty_pipe='/tmp/out pipe'; . '/tmp/init file'\n",
        )
        self.assertEqual(
            get_adapter("csh").source(*values),
    ...
    def test_refresh_preserves_status_before_source_only_when_required(self):
        """Add status-saving commands only to adapters requiring explicit refresh."""
        directory = Path("/tmp/ish scripts")
        self.assertEqual(
            get_adapter("csh").refresh_command(directory),
            f"set _ish_shell_exit_code = $status; source /tmp/ish\\ scripts/{CSH_UPDATE_SCRIPT}\n",
        )
        self.assertEqual(
            get_adapter("sh").refresh_command(directory),
    ...
    def test_executable_aliases_and_csh_symlink(self):
        """Select the correct adapter for executable aliases and csh symlinks targeting
        tcsh.
        """
    ...
    def test_new_shell_reuses_family_quoting_aliases_and_builtins(self):
        """Reuse quoting, alias parsing, and builtin handling by registering a shell with
        existing syntax.
        """
    ...
    def test_new_syntax_supplies_source_assignment_and_status_expression(self):
        """Provide consistent source, assignment, and status expressions in a new syntax
        profile.
        """
        syntax = ShellSyntax(
            ...
            exit_status="$result",
            source_command="load",
            parse_aliases=lambda text: {"custom": text},
            lexer="text",
        )
        adapter = ShellAdapter(
            ...
            ("-i",),
            "init.example",
            source_args=("input", "output"),
            refresh_script="refresh.example",
            capture_refresh_status=True,
        )
        with (
            patch.dict(SYNTAXES, {"example": syntax}),
            patch.dict(ADAPTERS, {"example": adapter}),
        ):
            self.assertEqual(
                adapter.source("/init", "/in pipe", "/out pipe"),
                "let input </in pipe>; let output </out pipe>; load </init>\n",
            )
            self.assertEqual(
                adapter.refresh_command(Path("/tmp")),
                "let _ish_shell_exit_code $result; load </tmp/refresh.example>\n",
            )
            self.assertEqual(
                alias_parser(b"value", "utf-8", "example"), {"custom": "value"}
            )

    def test_native_input_policy_can_be_reused_and_overridden(self):
        """Reuse or override input policy without changing syntax."""
        ...
        for native in (True, False):
            ...
            with (
                ...
                patch.dict(ADAPTERS, {"new-shell": adapter}),
            ):
                session = Mock()
                ...
                if native:
                    ...
                    self.assertEqual(
                        adapter.behavior.split_commands(command), [command]
                    )
                else:
                    self.assertIsNone(output)
                    session.set_prompt.assert_called_once_with(
                        b"foreach? ", engine.encoder
                    )
                    self.assertTrue(engine.prompt_event.is_set())
                    self.assertEqual(
                        adapter.behavior.split_commands(command), command.splitlines()
                    )


class AdapterSignalTests(unittest.TestCase):
    """Verify that shell templates and the sequencer use the same session signals."""

    def test_configured_signals_at_every_chunk_boundary(self):
        # A renamed adapter must keep the extra caret prompt registration.
        """Invoke the same adapter callbacks at every possible signal chunk boundary."""
            ...
            for split in range(len(stream) + 1):
                with self.subTest(shell=adapter.name, split=split):
                    ...
                    adapter.configure_sequencer(
                        sequencer,
                        prompt_id=ids.append,
                        prompt=prompts.append,
                        continuation=continuations.append,
                        unhooked_prompt=recovered.append,
                    )
                    output = sequencer.interpret(stream[:split]) + sequencer.interpret(
                        stream[split:]
                    )
    ...
    def test_script_markers_use_shared_constants(self):
        ...
        """Match script boundary markers to shared constants."""
        start, prefix = b"\x1b]900;X\a", b"\x1b]900;I;"
        with (
            patch("ish.shell.scripts.BEFORE_PROMPT", start),
            patch("ish.shell.scripts.PROMPT_ID_PREFIX", prefix),
        ):
