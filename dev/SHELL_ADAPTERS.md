...
- `SYNTAXES`: 문법별 인용 함수, 변수 대입 형식, 종료 코드 표현식, 스크립트 로딩 명령, alias 파서, 구문 강조 lexer.
...
`ShellBehavior`는 문법과 별도로 선택합니다. C shell 계열의 기본 정책은 보조 프롬프트를 셸에 맡기고, 선입력 표시를 유지하며, 여러 줄을 한 번에 보내고, 프로세스의 유휴 상태를 근거로 복구 명령을 주입하지 않는 것입니다. 같은 문법을 쓰는 셸이라도 필요한 정책을 개별적으로 설정할 수 있습니다.
...
`src/ish/shell/constants.py`의 `PROMPT_ID_PREFIX`, `BEFORE_PROMPT`, `AFTER_PROMPT`, `CARET_BEFORE_PROMPT` 등의 상수를 송수신 양쪽에서 사용합니다. 셸 템플릿에 필요한 printf 이스케이프도 이 바이트 상수에서 생성합니다.
